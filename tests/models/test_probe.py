...
class TestProbeModel:
    async def test_create_probe(self, db_session):
    ...
    async def test_create_probe_with_invalid_direction(self, db_session):
    ...
    async def test_create_probe_with_invalid_size(self, db_session, x, y):
        probe = Probe(size_x=x, size_y=y, direction="NORTH")
        db_session.add(probe)
