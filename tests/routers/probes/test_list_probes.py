...
@pytest.mark.asyncio
class TestListProbe:
    async def test_list_probes_empty(self, app_client):
    ...
    async def test_list_probes_with_single_data(self, app_client, probe_a):
        response = app_client.get("/probes")
        assert response.status_code == 200
    ...
    async def test_list_probes_with_multiple_data(self, app_client, probe_a, probe_b):
        response = app_client.get("/probes")
        assert response.status_code == 200
